...
ov::Tensor InputsEmbedderQwen3VL::get_interpolated_pos_embeds(
    ...
    const size_t num_positions = pos_embeds.get_shape()[1];
    const size_t embed_dim = pos_embeds.get_shape()[2];
    const size_t spatial_merge_size = m_vision_encoder->get_processor_config().merge_size;

    const float* pos_embeds_data = pos_embeds.data<const float>();
    const float* weights_data = weights.data<const float>();

    // Build spatial merge permutation: output_idx -> source position index
    std::vector<size_t> perm_indices;
    perm_indices.reserve(num_positions);
                ...
                for (size_t mwi = 0; mwi < merge_w; ++mwi) {
                    for (size_t shi = 0; shi < spatial_merge_size; ++shi) {
                        for (size_t swi = 0; swi < spatial_merge_size; ++swi) {
                            const size_t src_h = mhi * spatial_merge_size + shi;
                            const size_t src_w = mwi * spatial_merge_size + swi;
                            perm_indices.push_back(offset + ti * hw + src_h * w + src_w);
                        }
                    }
                }
            }
    ...
    // Fused weighted sum + spatial merge permute in single pass
    // Eliminates intermediate weighted_sum tensor, zero-fill, permuted_data vector, and final memcpy
    ov::Tensor result{ov::element::f32, {num_positions, embed_dim}};
    float* result_data = result.data<float>();

    for (size_t out_idx = 0; out_idx < num_positions; ++out_idx) {
        const size_t src_pos = perm_indices[out_idx];
        float* dst = result_data + out_idx * embed_dim;

        // First corner: direct assign (avoids zero-fill + add)
        {
            const float w = weights_data[src_pos];
            const float* src = pos_embeds_data + src_pos * embed_dim;
            for (size_t d = 0; d < embed_dim; ++d) {
                dst[d] = w * src[d];
            }
        }

        // Remaining 3 corners: accumulate
        for (size_t corner = 1; corner < 4; ++corner) {
            const float w = weights_data[corner * num_positions + src_pos];
            const float* src = pos_embeds_data + (corner * num_positions + src_pos) * embed_dim;
            for (size_t d = 0; d < embed_dim; ++d) {
                dst[d] += w * src[d];
            }
        }
    ...
    std::cout << "[TIMING] get_interpolated_pos_embeds (positions=" << num_positions << ", dim=" << embed_dim << "):\n"
              << "  indices_and_weights:    " << std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0).count() << " us\n"
              << "  pos_model_infer:        " << std::chrono::duration_cast<std::chrono::microseconds>(t2 - t1).count() << " us\n"
              << "  weighted_sum+permute:   " << std::chrono::duration_cast<std::chrono::microseconds>(t3 - t2).count() << " us\n"
              << "  TOTAL:                  " << std::chrono::duration_cast<std::chrono::microseconds>(t3 - t0).count() << " us" << std::endl;
...
std::pair<ov::Tensor, ov::Tensor> InputsEmbedderQwen3VL::run_video_image_embeddings_merger(
    ...
    const ov::Tensor& vision_embeds_ref = vision_embeddings_merger.get_tensor("last_hidden_state");
    m_lm_extra_inputs["deepstack_visual_embeds"] = vision_embeddings_merger.get_tensor("deepstack_feature_lists");
    
    auto vision_embeds_shape = vision_embeds_ref.get_shape();
    
    // Single contiguous copy of vision embeds, then split via zero-copy ROI views
    ov::Tensor owned_vision_embeds{vision_embeds_ref.get_element_type(), vision_embeds_shape};
    std::memcpy(owned_vision_embeds.data(), vision_embeds_ref.data(), owned_vision_embeds.get_byte_size());
    
    // Split vision embeddings via zero-copy ROI views on the owned tensor
    ...
    ov::Tensor video_embeds{owned_vision_embeds, {0, 0}, {video_count, vision_embeds_shape[1]}};
    ov::Tensor image_embeds{owned_vision_embeds, {video_count, 0}, {video_count + image_count, vision_embeds_shape[1]}};
    auto tm5 = std::chrono::steady_clock::now();

    std::cout << "[TIMING] run_video_image_embeddings_merger:\n"
              << "  reorder+concat:         " << std::chrono::duration_cast<std::chrono::microseconds>(tm1 - tm0).count() << " us\n"
              << "  interpolated_pos_embeds:" << std::chrono::duration_cast<std::chrono::microseconds>(tm2 - tm1).count() << " us\n"
              << "  get_rotary_pos_emb:     " << std::chrono::duration_cast<std::chrono::microseconds>(tm3 - tm2).count() << " us\n"
              << "  merger_infer:           " << std::chrono::duration_cast<std::chrono::microseconds>(tm4 - tm3).count() << " us\n"
              << "  split_embeds:           " << std::chrono::duration_cast<std::chrono::microseconds>(tm5 - tm4).count() << " us\n"
              << "  TOTAL:                  " << std::chrono::duration_cast<std::chrono::microseconds>(tm5 - tm0).count() << " us" << std::endl;
...
ov::Tensor InputsEmbedderQwen3VL::get_inputs_embeds(
    ...
    auto te2 = std::chrono::steady_clock::now();
    ...
    auto te3 = std::chrono::steady_clock::now();
    ...
    std::cout << "[TIMING] get_inputs_embeds (seq_len=" << input_ids.get_shape().at(1) << "):\n"
              << "  tokenize+text_embed:    " << std::chrono::duration_cast<std::chrono::microseconds>(te1 - te0).count() << " us\n"
              << "  create_position_ids:    " << std::chrono::duration_cast<std::chrono::microseconds>(te2 - te1).count() << " us\n"
              << "  vision_merger:          " << std::chrono::duration_cast<std::chrono::microseconds>(te3 - te2).count() << " us\n"
              << "  merge_text_vision:      " << std::chrono::duration_cast<std::chrono::microseconds>(te4 - te3).count() << " us\n"
              << "  TOTAL:                  " << std::chrono::duration_cast<std::chrono::microseconds>(te4 - te0).count() << " us" << std::endl;
