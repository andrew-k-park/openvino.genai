                ...
                for (size_t mwi = 0; mwi < merge_w; ++mwi) {
                    for (size_t shi = 0; shi < spatial_merge_size; ++shi) {
                        for (size_t swi = 0; swi < spatial_merge_size; ++swi) {
                            ...
                            const size_t src_idx = src_grid_offset + ti * hw + src_h * w + src_w;

                            std::memcpy(result_data + dst_offset * embed_dim,
 ...
 * 
 * This eliminates CPU weighted sum and reduces GPU→CPU transfer size by 4x.
 */
...
ov::Tensor InputsEmbedderQwen3VL::get_interpolated_pos_embeds(
    ...
    ov::Tensor weighted_sum = vision_embeddings_pos.get_output_tensor();
    // weighted_sum shape: [N, D] — weighted sum computed inside the model
